...
from core.slack.naming import BOX_FOLDER_LINK_RE
...
def _announce_new_experiment(client, meta, suffix, associations, renamed_list=()):
    ...
    `meta` carries value ("category:dir_key"), user, channel, response_url."""
    ...
    for renamed in renamed_list:
        post_legacy_rename(post_message, meta["user"], renamed)
    finish_new_experiment(
        category,
        dir_key,
        meta["user"],
...
def register(app):
    ...
    def handle_directory_pick(ack, body, respond):
        """Step 3 of /experiment new: create immediately or add details."""
        ack()
        value = body["actions"][0]["value"]  # "category:dir_key"
        category, dir_key = value.split(":", 1)
        directory = box_client.directory_info(dir_key)
        extras = (
            "scan count, associated experiment"
            if dir_key == "scans"
            else "model codename, associated experiment"
        )
        respond(
            response_type="ephemeral",
            replace_original=True,
            text="Create it now, or add details first?",
            blocks=[
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                        "text": (
                            f":test_tube: *New {category.upper()} experiment* in "
                            f"_{directory['label']}_ — create it now, or add "
                            f"details ({extras})?"
                        ),
                    },
                },
                {
                    "type": "actions",
                    "block_id": "experiment_details_choice",
                    "elements": [
                        {
                            "type": "button",
                            "action_id": "create_now",
                            "text": {"type": "plain_text", "text": "Create now"},
                            "style": "primary",
                            "value": value,
                        },
                        {
    ...
    @app.action("create_now")
    def handle_create_now(ack, body, respond, say):
        """No extras: generate and announce right away."""
        ack()
        category, dir_key = body["actions"][0]["value"].split(":", 1)
        finish_new_experiment(
            category,
            dir_key,
            body["user"]["id"],
            post_message=say,
            update_ephemeral=lambda text: respond(
                response_type="ephemeral", replace_original=True, text=text
            ),
        )

    @app.action("add_details")
    def handle_add_details(ack, body, client, respond):
        """Open the details dialog: scan count / codename + association."""
        ack()
        value = body["actions"][0]["value"]  # "category:dir_key"
    ...
    def handle_details_submit(ack, view, client):
        """Details dialog submitted: build the suffix and classify each
        association entry.

        Associations that resolve cleanly (named experiments, combos,
        ...
        # Directory-specific optional suffix.
        suffix = ""
        if "scan_count" in vals:
            count = vals["scan_count"]["count_input"].get("value")
            if count:
                suffix = f"-({int(count)})"
        elif "codename" in vals:
            raw_code = (vals["codename"]["codename_input"]["value"] or "").strip()
            if raw_code:
                ...
                if not re.fullmatch(r"[A-Za-z0-9._-]+", code):
                    ack(
                        ...
                        errors={
                            "codename": "Letters, digits, dots, dashes and "
                            "underscores only."
                        },
                    )
                    return
        ...
        ack()
        _announce_new_experiment(client, meta, suffix, ready)
    ...
    def handle_associate_legacy_submit(ack, view, client):
        ...
        for i, lg in enumerate(meta["legacy"]):
            ...
            try:
                renamed = convert_legacy_folder(
                    lg["folder_id"], picked_date(i), picked_category(i)
                )
        ...
        _announce_new_experiment(
            client, meta, meta["suffix"], associations, renamed_list
        )
    ...
    def handle_legacy_submit(ack, view, client):
        ...
        explicit user input taking precedence over detection.
        """
        meta = json.loads(view["private_metadata"])
        vals = view["state"]["values"]
        ...
        try:
            renamed = convert_legacy_folder(link.group(1), picked_date, picked_category)
        except LegacyConversionError as e:
        ...
        post_legacy_rename(
            lambda **kw: client.chat_postMessage(channel=meta["channel"], **kw),
            meta["user"],
            renamed,
        )
